...
for eng in sys.argv[1:]:
    ...
    _zc = rt.pinned_input_slots(len(frames))      # zero-copy: pre-stage into slots
    for _k, _fr in enumerate(frames):
        _zc[_k][...] = _fr[0]
    frames = [(_zc[_k],) + tuple(_fr[1:]) for _k, _fr in enumerate(frames)]
    print("[bench] zero-copy input slots", len(_zc))
