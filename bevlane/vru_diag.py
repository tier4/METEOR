...
def run(tag, ckpt, model, n_cams, scenes):
    ...
    for i in range(0, len(ds), max(1, len(ds) // 200)):
        ...
        hm = out[3].float()
        dets = m.decode_boxes(hm, out[4].float(), thresh=0.25)[0]
        det_by_cls = {0: [], 1: []}
        for d in dets:
...
if __name__ == "__main__":
    ...
    run("v55 full-grid", "out/v55_remote_best.pt", "v55", 8, scenes)
    run("v59 rear-40", "out/v59_remote_last.pt", "v55", 7, scenes)
