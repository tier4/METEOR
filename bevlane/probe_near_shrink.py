"""交差点接近に伴う「自車直近の路面の欠け」を帯別・時系列で測る (2026-08-22)。

0-20m 一括では交差点で面積が増えて見え、ユーザーが観察した「近くの道路が
しぼむ」を捉えられなかった。自車直近 (0-5m) は 8 カメラでも俯角が大きく
死角になりやすいので、0-5 / 5-10 / 10-20 m に分けて、交差点までの距離
(前方の GT crosswalk 最近傍距離) と対応付ける。
"""
...
DIST = [("交差点 0-10m", 0, 10), ("10-20m", 10, 20),
        ("20-40m", 20, 40), ("交差点なし", 40, 999)]
...
for s, lst in list(by_scene.items())[:a.scenes]:
    ...
    for fi, di in lst[::3]:
        ...
        cw = np.argwhere(gt[:400, COLS] == 3)      # 前方の横断歩道
        d_ix = 999.0
        if len(cw):
            d_ix = float((400 - cw[:, 0].max()) * 0.2)   # 最も近い横断歩道
...
print(f"\n=== {a.tag} 交差点までの距離 x 帯別 road 被覆率 (pred / GT) ===")
print("交差点距離        0-5m              5-10m             10-20m")
for dn, _, _ in DIST:
    row = f"  {dn:12s}"
    for bn, _, _ in BANDS:
        v = acc[(dn, bn)]
