#!/usr/bin/env python3
"""Dense depth GT for the two NARROW (telephoto) cams -> depth_gt4n/. 
...
- Saved per frame as depth_gt4n/<fi>.npz (float16 [6,72,128], 0 = invalid);
  manifest frames gain a "depth4n" key.
"""
import argparse
import base64
import json
...
CAMS = ["CAM_FRONT_NARROW", "CAM_BACK_NARROW"]
...
# ground-plane classes: geometric fill (ray ∩ ego z=0) where LiDAR is absent
...
def seg_map_small(anns, hw_full):
    """Segment-id map at (DH, DW); returns (seg_ids int32, sky_mask, ego_mask)."""
...
def densify(sparse, seg, sky, ego, gnd=None, gpl=None):
    ...
    if invalid.any() and (d > 0).any():
        ...
        # 1) smooth 2-D linear interpolation over the merged road(+paint)
        if gnd is not None:
            src_m = (d > 0) & gnd
            tgt_m = invalid & gnd
            if src_m.sum() >= 16 and tgt_m.any():
                ...
                d[tgt_m] = out
                invalid = d <= 0
        # 2) ground-plane fill outside the LiDAR hull (strictly downward rays)
        if gnd is not None and gpl is not None and invalid.any():
            fill = invalid & gnd & (gpl > 0)
            d[fill] = gpl[fill]
            invalid = d <= 0
        # 3) same-segment nearest fill for the rest
...
def process_scene(args):
    ...
    try:
        ...
        os.makedirs(os.path.join(out_dir, "depth_gt4n"), exist_ok=True)
        for fr in man["frames"]:
            fi = fr["frame"]
            path = os.path.join(out_dir, f"depth_gt4n/{fi:04d}.npz")
            if os.path.exists(path):
                fr["depth4n"] = f"depth_gt4n/{fi:04d}.npz"
                continue
            s = strided[fi]
