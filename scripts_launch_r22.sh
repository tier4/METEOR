#!/bin/bash
# r22: turn oversampling x3, mode-CE 0.6, shadow-filtered OCC GT.
# Waits for r21, then waits for the occ shadow filter to finish.
B=/home/umedan/work/BevLane
PY=/home/umedan/comet_venv/bin/python3
TR=/home/umedan/comet_venv/bin/torchrun
while pgrep -f "ckpt_r21 " > /dev/null || pgrep -f "bevlane_ckpt_r21$" > /dev/null || pgrep -f "out/bevlane_ckpt_r21" > /dev/null; do sleep 120; done
until grep -q '^DONE' $B/out/occ_filter_log.txt 2>/dev/null; do sleep 120; done
sleep 60
$PY - <<'PYEOF'
import json, os
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 nohup $TR --nproc_per_node=8 \
  ...
  --train-bg --aug --lr 5e-5 --val-every 500 --seed-subset 22 \
  --turn-oversample 3.0 \
  --init-ckpt $B/out/bevlane_ckpt_r21/last.pt \
  ...
  --out $B/out/bevlane_ckpt_r22 > $B/out/train_r22_log.txt 2>&1 &
echo "r22 launched $(date)" >> $B/out/r22_launcher.log
